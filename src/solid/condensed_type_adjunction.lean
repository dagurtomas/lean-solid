...
begin
  ...
  intros x,
  have h : π (ulift.down.{u+1 u} x.1.1) = π (ulift.down.{u+1 u} x.1.2),
  { cases x,
    cases x_val,
    cases x_val_snd,
    cases x_val_fst,
...
def is_compactly_generated_Top (X : Top.{u+1}) : Prop :=
  ...
  -- the above definition is wrong as is:
  -- do we want Π or ⨆?
  -- we don't want the topology to be equal to the coinduced,
  -- we want the it to be finer/coarser...
